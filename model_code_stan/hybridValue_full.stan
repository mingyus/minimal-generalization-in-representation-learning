...
transformed parameters {
        ...
        for (i in 1:Ns) {
                ...
                w4[i]  = Phi_approx( mu_pr[2] + sigma[2] * w4_samp[i] );
                eta[i] = Phi_approx( mu_pr[3] + sigma[3] * eta_samp[i] );
...
model {
        ...
        vector[4] Q4; // the four states are: [left,right,leftWrong,rightWrong]
        matrix[3,2] Q6;
        real delta4;
        real delta6;
        int currentSubject;
        ...
        for (tr in 1:Nt) {
            ...
            if (trialType[tr]!=0){ // valid trials or early exit trials (trials with choices)
                ...
                choice[tr] ~ categorical( (1-lapse[currentSubject]) * softmax( beta[currentSubject] * (w4[currentSubject]*Q4[odorchoice2state[odor[tr]]] + (1-w4[currentSubject])*Q6[odor[tr]]' + sb[currentSubject]*[0,1]' + perseveration) ) + lapse[currentSubject]/2 );

                // calculate reward prediction error
                delta4 = reward[tr] * pow(gamma[currentSubject], delay[tr]) - Q4[odorchoice2state[odor[tr], choice[tr]]];
                delta6 = reward[tr] * pow(gamma[currentSubject], delay[tr]) - Q6[odor[tr], choice[tr]];
                
                // update Q values
                Q4[odorchoice2state[odor[tr], choice[tr]]] += eta[currentSubject] * delta4;
                Q6[odor[tr], choice[tr]] += eta[currentSubject] * delta6;
